...
public class LoginFilter implements Filter {
	...
	public LoginFilter() {
		ignoreUrls.put("/jingbao", Matcher.STARTS_WIDTH);
		ignoreUrls.put("/index", Matcher.EQUALS);
		ignoreUrls.put("/register", Matcher.EQUALS);
		ignoreUrls.put("/login", Matcher.EQUALS);
		ignoreUrls.put(".css", Matcher.ENDS_WIDTH);
		ignoreUrls.put(".woff2", Matcher.ENDS_WIDTH);
		ignoreUrls.put(".woff", Matcher.ENDS_WIDTH);
		ignoreUrls.put(".ttf", Matcher.ENDS_WIDTH);
		ignoreUrls.put(".jpg", Matcher.ENDS_WIDTH);
		ignoreUrls.put(".png", Matcher.ENDS_WIDTH);
		ignoreUrls.put(".js", Matcher.ENDS_WIDTH);
		ignoreUrls.put(".gif", Matcher.ENDS_WIDTH);
	}
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		for (String key : ignoreUrls.keySet()) {
			...
			}else if(v == Matcher.STARTS_WIDTH){
